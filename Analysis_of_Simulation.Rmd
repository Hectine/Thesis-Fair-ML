...
```{r}
# set data
data <- read.csv("balanced_non_linear.csv")
# "scarce_non_linear.csv"
# "balanced_non_linear.csv"
# "balanced_linear.csv"
# "scarce_linear.csv"
...
# 3.2 Imputation method for missing data
...
NA_method <- "Miss_Forest_Imputation"
...
# 6. Initialise new data frame to store results
initialize_df = FALSE
...
table(data_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "Yes",]$crime)
table(data_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "No",]$crime)
table(data_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "Yes",]$crime)
table(data_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "No",]$crime)
table(data_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "Yes",]$crime)
table(data_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "No",]$crime)
table(data_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "Yes",]$crime)
table(data_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "No",]$crime)
```
...
# Stringent Threshold
    ...
    threshold_stringent <- quantile(probabilities_train, 0.1)
    
    # predictions
    prediction_test_stringent <- ifelse(probabilities_test >= threshold_stringent, 1, 0)
    prediction_train_stringent <- ifelse(probabilities_train >= threshold_stringent, 1, 0)
    
    # confusion matrix
    cm_train_stringent <- table(data_train$crime, prediction_train_stringent)
    cm_test_stringent <- table(data_test$crime, prediction_test_stringent)
    
# Lenient threshold
    # obtain threshold value
    threshold_lenient <- quantile(probabilities_train, 0.9)
    if(threshold_lenient == 1){
      threshold_lenient <- quantile(probabilities_test, 0.95)
    }
    
    # predictions
    prediction_test_lenient <- ifelse(probabilities_test >= threshold_lenient, 1, 0)
    prediction_train_lenient <- ifelse(probabilities_train >= threshold_lenient, 1, 0)
    
    # confusion matrix
    cm_train_lenient <- table(data_train$crime, prediction_train_lenient)
    cm_test_lenient <- table(data_test$crime, prediction_test_lenient)
```
...
# Adjust cm_test
if(dim(cm_test)[2] == 1){
  expanded_cm_test <- matrix(0, nrow = 2, ncol = 2, dimnames = list(c("0", "1"), c("0", "1")))
  expanded_cm_test[, "0"] <- cm_test
  cm_test <- expanded_cm_test
}
...
## AUC for subgroups
```{r}
# Function to calculate AUC with error handling
calculate_auc_safe <- function(data, probabilities) {
  tryCatch({
    roc_curve <- roc(data$crime, probabilities)
    auc(roc_curve)
  }, error = function(e) {
...
auc_value_m_maj_ndis <- calculate_auc_safe(
  data_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "No",], 
  probabilities_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "No"]
)

# Male Majority disabled
auc_value_m_maj_dis <- calculate_auc_safe(
  data_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "Yes",], 
  probabilities_test[data_test$gender == "Male" & data_test$ethnicity == "Majority" & data_test$disability == "Yes"]
)

# Male Minority non-disabled
auc_value_m_min_ndis <- calculate_auc_safe(
  data_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "No",], 
  probabilities_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "No"]
)

# Male Minority disabled
auc_value_m_min_dis <- calculate_auc_safe(
  data_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "Yes",], 
  probabilities_test[data_test$gender == "Male" & data_test$ethnicity == "Minority" & data_test$disability == "Yes"]
)

# Female Majority non-disabled
auc_value_f_maj_ndis <- calculate_auc_safe(
  data_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "No",], 
  probabilities_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "No"]
)

# Female Majority disabled
auc_value_f_maj_dis <- calculate_auc_safe(
  data_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "Yes",], 
  probabilities_test[data_test$gender == "Female" & data_test$ethnicity == "Majority" & data_test$disability == "Yes"]
)

# Female Minority non-disabled
auc_value_f_min_ndis <- calculate_auc_safe(
  data_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "No",], 
  probabilities_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "No"]
)

# Female Minority disabled
auc_value_f_min_dis <- calculate_auc_safe(
  data_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "Yes",], 
  probabilities_test[data_test$gender == "Female" & data_test$ethnicity == "Minority" & data_test$disability == "Yes"]
)
...
if(sum(cm_test[,2]) == 0){
  FPR_benchmark_test <- 0
  FNR_benchmark_test <- 1
}

if(ncol(cm_test_stringent) == 2) {
  # Metrics for stringent threshold
  FPR_stringent <- cm_test_stringent[1,2] / (cm_test_stringent[1,2] + cm_test_stringent[1,1])
  FNR_stringent <- cm_test_stringent[2,1] / (cm_test_stringent[2,1] + cm_test_stringent[2,2])
} else {
  FPR_stringent <- 1
  FNR_stringent <- 0
}

if(ncol(cm_test_lenient) == 2) {
  # Metrics for lenient threshold
  FPR_lenient <- cm_test_lenient[1,2] / (cm_test_lenient[1,2] + cm_test_lenient[1,1])
  FNR_lenient <- cm_test_lenient[2,1] / (cm_test_lenient[2,1] + cm_test_lenient[2,2])
} else {
  FPR_lenient <- 0
  FNR_lenient <- 1
}
...
calculate_metrics <- function(cm, cm_benchmark) {
  ...
  FPR <- NA
  FNR <- NA
  proportion_favorable_benchmark <- NA
  FPR_benchmark <- NA
  FNR_benchmark <- NA
  ...
  if (all(dim(cm) == c(2, 2))) {
    ...
    if (sum(cm) == sum(cm[,1])) {
      FPR <- 0  # All predictions are negative
      FNR <- 1
    } else if ((cm[1, 2] + cm[1, 1]) > 0) {
      FPR <- cm[1, 2] / (cm[1, 2] + cm[1, 1])
    }

    # Calculate False Negative Rate (FNR)
    if (sum(cm[, 1]) == 0) {
      FNR <- 0  # All predictions are positive
      FPR <- 1
    } else if ((cm[2, 1] + cm[2, 2]) > 0) {
  ...
  if (all(dim(cm_benchmark) == c(2, 2))) {
    # Calculate Benchmark metrics
    proportion_favorable_benchmark <- sum(cm_benchmark[, 1]) / sum(cm_benchmark)

    if (sum(cm_benchmark[1, ]) == 0) {
      FPR_benchmark <- 0  # All benchmark predictions are negative
    } else {
      FPR_benchmark <- cm_benchmark[1, 2] / (cm_benchmark[1, 2] + cm_benchmark[1, 1])
    }

    if (sum(cm_benchmark[, 2]) == 0) {
      FNR_benchmark <- 0  # All benchmark predictions are positive
    } else {
  ...
  FPR_ratio <- if (!is.na(FPR) && !is.na(FPR_benchmark) && FPR_benchmark != 0) FPR / FPR_benchmark else NA
  FNR_ratio <- if (!is.na(FNR) && !is.na(FNR_benchmark) && FNR_benchmark != 0) FNR / FNR_benchmark else NA

  return(list(proportion_favorable = proportion_favorable, 
              proportion_favorable_ratio = proportion_favorable_ratio,
              FPR = FPR, 
              FPR_ratio = FPR_ratio, 
              FNR = FNR, 
              FNR_ratio = FNR_ratio))
...
## Fine grained subpopulations for stringent and lenient threshold
```{r}
# Lenient threshold
  # Create a list to store metrics for all subpopulations
  subpop_metrics_test_lenient <- list()
  ...
  for (gender in c("Male", "Female")) {
    for (ethnicity in c("Minority", "Majority")) {
      for (disability in c("Yes", "No")) {
        ...
        # Confusion matrix
        cm_test_subpop <- table(data_test[indices_test,]$crime, prediction_test_lenient[indices_test])
        
        # Calculate metrics
        metrics_test <- calculate_metrics(cm_test_subpop, cm_test_lenient)
        
        # Store metrics
        subpop_metrics_test_lenient[[subpop]] <- metrics_test
      }
    }
  }
...
# Stringent threshold
  # Create a list to store metrics for all subpopulations
  subpop_metrics_test_stringent <- list()
  ...
  for (gender in c("Male", "Female")) {
    for (ethnicity in c("Minority", "Majority")) {
      for (disability in c("Yes", "No")) {
        ...
        # Confusion matrix
        cm_test_subpop <- table(data_test[indices_test,]$crime, prediction_test_stringent[indices_test])
        
        # Calculate metrics
        metrics_test <- calculate_metrics(cm_test_subpop, cm_test_stringent)
        
        # Store metrics
        subpop_metrics_test_stringent[[subpop]] <- metrics_test
      }
    }
  }
```
...
if(is.null(extremes_test$most_privileged$FPR$value)){
  FPR_80 <- 0
}
...
if(is.null(extremes_test$most_disadvantaged$FNR$value)){
  FNR_80 <- 0
}
...
if(is.null(extremes_test$most_privileged$Demographic_Parity$value)){
  parity_80 <- 0
}
...
This code dynamically gathers model performance and fairness metrics, storing them in a structured table. The design supports easy comparison across different model configurations and demographic subpopulations, which is helpful for analyzing both accuracy and fairness in machine learning models.

The list contains several categories:
*Model performance metrics:* Includes auc, accuracy_test, and accuracy_train to measure the overall model performance.
*Benchmark metrics:* Comprises fairness-related metrics such as the proportion of favorable outcomes and false positive/negative rates (FPR, FNR) for benchmark comparisons.
*Single attribute comparisons:* Captures fairness metrics across specific demographic groups like gender (male, female), disability status (disabled, abled), and ethnicity (minority, majority). Each demographic group's favorable outcome rate and error rates (FPR, FNR) are recorded.
*Subpopulation metrics:* Extends the fairness analysis by comparing combinations of demographic groups (e.g., Male_Minority_Yes, Female_Majority_No). These combinations have their respective favorable proportions, FPR, FNR, and corresponding ratios.
...
extracted_metrics <- list(
  ...
  # AUC values for subgroups
  AUC_male_majority_nondisabled = auc_value_m_maj_ndis,
  AUC_male_majority_disabled = auc_value_m_maj_dis,
  AUC_male_minority_nondisabled = auc_value_m_min_ndis,
  AUC_male_minority_disabled = auc_value_m_min_dis,
  AUC_female_majority_nondisabled = auc_value_f_maj_ndis,
  AUC_female_majority_disabled = auc_value_f_maj_dis,
  AUC_female_minority_nondisabled = auc_value_f_min_ndis,
  AUC_female_minority_disabled = auc_value_f_min_dis,
  ...
  FPR_train = FPR_benchmark_train,
  FNR_train = FNR_benchmark_train,
  
  # Benchmark metrics for lenient threshold
  FPR_lenient = FPR_lenient,
  FNR_lenient = FNR_lenient,
  
  # Benchmark metrics for stringent threshold
  FPR_stringent = FPR_stringent,
  FNR_stringent = FNR_stringent,
  ...
  # Metrics from stringent threshold
      # Male_Minority_Yes
    Male_Minority_Yes_FPR_stringent = subpop_metrics_test_stringent$Male_Minority_Yes$FPR,
    Male_Minority_Yes_FNR_stringent = subpop_metrics_test_stringent$Male_Minority_Yes$FNR,
    
    # Male_Minority_No
    Male_Minority_No_FPR_stringent = subpop_metrics_test_stringent$Male_Minority_No$FPR,
    Male_Minority_No_FNR_stringent = subpop_metrics_test_stringent$Male_Minority_No$FNR,
    
    # Male_Majority_Yes
    Male_Majority_Yes_FPR_stringent = subpop_metrics_test_stringent$Male_Majority_Yes$FPR,
    Male_Majority_Yes_FNR_stringent = subpop_metrics_test_stringent$Male_Majority_Yes$FNR,
    
    # Male_Majority_No
    Male_Majority_No_FPR_stringent = subpop_metrics_test_stringent$Male_Majority_No$FPR,
    Male_Majority_No_FNR_stringent = subpop_metrics_test_stringent$Male_Majority_No$FNR,
    
    # Female_Minority_Yes
    Female_Minority_Yes_FPR_stringent = subpop_metrics_test_stringent$Female_Minority_Yes$FPR,
    Female_Minority_Yes_FNR_stringent = subpop_metrics_test_stringent$Female_Minority_Yes$FNR,
    
    # Female_Minority_No
    Female_Minority_No_FPR_stringent = subpop_metrics_test_stringent$Female_Minority_No$FPR,
    Female_Minority_No_FNR_stringent = subpop_metrics_test_stringent$Female_Minority_No$FNR,
    
    # Female_Majority_Yes
    Female_Majority_Yes_FPR_stringent = subpop_metrics_test_stringent$Female_Majority_Yes$FPR,
    Female_Majority_Yes_FNR_stringent = subpop_metrics_test_stringent$Female_Majority_Yes$FNR,
    
    # Female_Majority_No
    Female_Majority_No_FPR_stringent = subpop_metrics_test_stringent$Female_Majority_No$FPR,
    Female_Majority_No_FNR_stringent = subpop_metrics_test_stringent$Female_Majority_No$FNR,

    # Metrics from lenient threshold
      # Male_Minority_Yes
    Male_Minority_Yes_FPR_lenient = subpop_metrics_test_lenient$Male_Minority_Yes$FPR,
    Male_Minority_Yes_FNR_lenient = subpop_metrics_test_lenient$Male_Minority_Yes$FNR,
    
    # Male_Minority_No
    Male_Minority_No_FPR_lenient = subpop_metrics_test_lenient$Male_Minority_No$FPR,
    Male_Minority_No_FNR_lenient = subpop_metrics_test_lenient$Male_Minority_No$FNR,
    
    # Male_Majority_Yes
    Male_Majority_Yes_FPR_lenient = subpop_metrics_test_lenient$Male_Majority_Yes$FPR,
    Male_Majority_Yes_FNR_lenient = subpop_metrics_test_lenient$Male_Majority_Yes$FNR,
    
    # Male_Majority_No
    Male_Majority_No_FPR_lenient = subpop_metrics_test_lenient$Male_Majority_No$FPR,
    Male_Majority_No_FNR_lenient = subpop_metrics_test_lenient$Male_Majority_No$FNR,
    
    # Female_Minority_Yes
    Female_Minority_Yes_FPR_lenient = subpop_metrics_test_lenient$Female_Minority_Yes$FPR,
    Female_Minority_Yes_FNR_lenient = subpop_metrics_test_lenient$Female_Minority_Yes$FNR,
    
    # Female_Minority_No
    Female_Minority_No_FPR_lenient = subpop_metrics_test_lenient$Female_Minority_No$FPR,
    Female_Minority_No_FNR_lenient = subpop_metrics_test_lenient$Female_Minority_No$FNR,
    
    # Female_Majority_Yes
    Female_Majority_Yes_FPR_lenient = subpop_metrics_test_lenient$Female_Majority_Yes$FPR,
    Female_Majority_Yes_FNR_lenient = subpop_metrics_test_lenient$Female_Majority_Yes$FNR,
    
    # Female_Majority_No
    Female_Majority_No_FPR_lenient = subpop_metrics_test_lenient$Female_Majority_No$FPR,
    Female_Majority_No_FNR_lenient = subpop_metrics_test_lenient$Female_Majority_No$FNR,
  
  # Subpopulation metrics for training data (measure of overfit)
    # Male_Minority_Yes
    Male_Minority_Yes_FPR_train = subpop_metrics_train$Male_Minority_Yes$FPR,
    Male_Minority_Yes_FNR_train = subpop_metrics_train$Male_Minority_Yes$FNR,
    
    # Male_Minority_No
    Male_Minority_No_FPR_train = subpop_metrics_train$Male_Minority_No$FPR,
    Male_Minority_No_FNR_train = subpop_metrics_train$Male_Minority_No$FNR,
    
    # Male_Majority_Yes
    Male_Majority_Yes_FPR_train = subpop_metrics_train$Male_Majority_Yes$FPR,
    Male_Majority_Yes_FNR_train = subpop_metrics_train$Male_Majority_Yes$FNR,
    
    # Male_Majority_No
    Male_Majority_No_FPR_train = subpop_metrics_train$Male_Majority_No$FPR,
    Male_Majority_No_FNR_train = subpop_metrics_train$Male_Majority_No$FNR,
    
    # Female_Minority_Yes
    Female_Minority_Yes_FPR_train = subpop_metrics_train$Female_Minority_Yes$FPR,
    Female_Minority_Yes_FNR_train = subpop_metrics_train$Female_Minority_Yes$FNR,
    
    # Female_Minority_No
    Female_Minority_No_FPR_train = subpop_metrics_train$Female_Minority_No$FPR,
    Female_Minority_No_FNR_train = subpop_metrics_train$Female_Minority_No$FNR,
    
    # Female_Majority_Yes
    Female_Majority_Yes_FPR_train = subpop_metrics_train$Female_Majority_Yes$FPR,
    Female_Majority_Yes_FNR_train = subpop_metrics_train$Female_Majority_Yes$FNR,
    
    # Female_Majority_No
    Female_Majority_No_FPR_train = subpop_metrics_train$Female_Majority_No$FPR,
    Female_Majority_No_FNR_train = subpop_metrics_train$Female_Majority_No$FNR
)
...
#write.csv(df_metrics, file = "AnalysisBalanced_Nonlinear.csv")
```
