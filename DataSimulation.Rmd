...
if(non_linear == TRUE){
  ...
  crime_increase_noise <- ifelse(crime_increase == 1, 
                                 rnorm(sum(crime_increase), mean = 0.5, sd = 0.5), 
                                 0)
  ...
  crime_decrease_noise <- ifelse(crime_decrease == -1, 
                                 rnorm(abs(sum(crime_decrease)), mean = -0.6, sd = 0.5), 
                                 0)
  ...
  crime_increase_education <- ifelse(simulated_data$education == "Master's" | simulated_data$education == "Doctorate", 
                                     sample(c(0,1), 100, replace = TRUE, prob = c(0.60, 0.40)), 
                                     0)
  crime_increase_education_noise <- ifelse(crime_increase_education == 1, 
                                 rnorm(sum(crime_increase_education), mean = 1, sd = 0.5), 
                                 0)
  ...
  crime_decrease_education_noise <- ifelse(crime_decrease_education == -1, 
                                 rnorm(abs(sum(crime_decrease_education)), mean = -0.5, sd = 0.5), 
                                 0)  
...
if(non_linear == FALSE) {
  ...
  set.seed(123) # Ensure reproducibility
  selected_above <- sample(above_threshold, size = floor(0.8 * length(above_threshold)), replace = FALSE)
  selected_below <- sample(bottom_20_below_threshold, size = floor(0.2 * length(above_threshold)), replace = FALSE)
